...
Outils pour alimenter le dataset images (Google Drive) : route sèche / route mouillée / route enneigée.

- **drive_add_images.py** : ajoute des images dans `Mon Drive/dataset_routes/{dry_road|wet_road|snowy_road}` avec normalisation (128×128, JPG), déduplication (md5 + phash) et log CSV.
...
```bash
python dataset_tools/drive_add_images.py --label dry_road --input /chemin/vers/images
```
...
```bash
cd dataset_tools
python drive_add_images.py --label dry_road --input ../mes_photos
```
