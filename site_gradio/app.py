...
DATA_DIR = PROJECT_ROOT / "datasets" / "transformed" / "nb" / "normalisee"
...
MODEL_CONFIG = {
    "Modèle linéaire": {
        ...
        "cache": CACHE_DIR / "linear_nb_normalisee.npz",
    },
    "MLP": {
    ...
    "RBF": {
        ...
        "cache": CACHE_DIR / "rbf_nb_normalisee.npz",
    },
}
...
def load_training_data(dtype: np.dtype = np.float32) -> tuple[np.ndarray, np.ndarray]:
    x_path = DATA_DIR / "X_train.f32bin"
    y_path = DATA_DIR / "y_train.i32bin"
    require_file(x_path, "Le jeu d'entraînement NB normalisé est absent.")
    require_file(y_path, "Les étiquettes d'entraînement sont absentes.")
...
def load_linear() -> dict[str, object]:
    ...
    X, y = load_training_data(np.float32)
    d = X.shape[1]
    W = np.zeros(3 * d, dtype=np.float32)
    b = np.zeros(3, dtype=np.float32)
    lib.fit(
        X.ctypes.data_as(pf), y.ctypes.data_as(pi), X.shape[0], d,
        30, ctypes.c_float(0.1), W.ctypes.data_as(pf), b.ctypes.data_as(pf),
    )
...
def load_rbf() -> dict[str, object]:
    ...
    if cache_path.exists():
        saved = np.load(cache_path)
        return {
            "centres": saved["centres"], "W": saved["W"],
    ...
    lib.entrainer.argtypes = [
        ...
        ctypes.c_double, ctypes.c_uint, pd, pd,
    ]
    lib.entrainer.restype = None

    X, y = load_training_data(np.float64)
    d = X.shape[1]
    nb_centres, gamma = min(100, X.shape[0]), 0.01
    ...
    np.savez_compressed(
        cache_path, centres=centres, W=W, d=d,
        nb_centres=nb_centres, gamma=gamma,
    )
    return {
        "centres": centres, "W": W, "d": d,
...
def preprocess_image(image: Image.Image, d: int, dtype: np.dtype) -> np.ndarray:
    ...
    return np.ascontiguousarray(gray.reshape(d).astype(dtype))
...
def prediction_details(
    ...
    return (
        f"**{model_name}** · NB normalisé · {loading_note}\n\n"
...
def predict(image: Image.Image | None, model_name: str) -> tuple[str, str]:
    ...
    try:
        ...
        d = int(model["d"])
        x = preprocess_image(image, d, np.float64)
