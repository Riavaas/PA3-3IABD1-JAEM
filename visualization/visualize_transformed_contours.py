#!/usr/bin/env python3
"""
Affiche des exemples depuis datasets/transformed/contours/(normalisee|non_normalisee).

Chaque ligne de X.npy = une image "contours" aplatie (H*W).
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Visualise 3 images contours depuis datasets/transformed.")
    ...
    base = project_root / "datasets" / "transformed" / "contours" / sub
    ...
    fig.suptitle(f"Contours/{sub} - {k} exemple(s)")
    plt.tight_layout()
    plt.show()
